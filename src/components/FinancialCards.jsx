...
import React, { useMemo } from "react";
import { fmt, num } from "../utils/utils.js";
import { useAuth } from "../context/AuthContext.jsx"; // <-- Imported useAuth
...
const GradientCard = ({
  ...
  prefix = "$",
  isNegative = false,
  isBlurred = false, // <-- Added isBlurred prop
}) => (
  // Reduced padding on small laptops (lg:p-4) to give the text more breathing room
  <div
    className={`relative overflow-hidden rounded-2xl p-3 sm:p-5 lg:p-4 xl:p-6 text-white shadow-lg bg-gradient-to-br ${gradientFrom} ${gradientTo} ${shadowColor} transition-transform hover:-translate-y-1 duration-300 flex flex-col justify-between`}
  >
    <div className="relative z-10">
      {/* Title scales down on medium screens so it doesn't crowd the number */}
      <h3 className="text-white/90 text-[9px] sm:text-[11px] lg:text-[9px] xl:text-[11px] font-bold tracking-wider uppercase mb-1.5 min-h-[2.5rem] flex items-start leading-tight">
      ...
      {/* Added blur-md and select-none to hide the text visually and prevent highlighting if isBlurred is true */}
      <div
        className={`text-lg sm:text-2xl md:text-3xl lg:text-lg xl:text-2xl 2xl:text-3xl font-extrabold tracking-tight break-words ${isBlurred ? "blur-md select-none opacity-80" : ""}`}
      >
        {loading ? (
          "..."
        ) : (
...
export default function FinancialCards({
  ...
  // <-- Initialize useAuth to determine if the user is a market manager
  const { user } = useAuth();
  const isMarketManager = user?.role === "market_manager";

  const { actualVariance, finalCashInHand, cashPickup } = useMemo(() => {
    const s = num(sales);
    const b = num(bank);
    const p = num(pickup);
    const e = num(expenses);
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      {/* TOP ROW */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 lg:gap-3 xl:gap-6">
        {/* Passed isBlurred prop tied to market_manager status */}
        <GradientCard
        ...
        <GradientCard
          ...
          amount={cashPickup}
          loading={loading}
          isNegative={cashPickup < 0}
          gradientFrom={cashPickup >= 0 ? "from-orange-400" : "from-red-500"}
          gradientTo={cashPickup >= 0 ? "to-amber-500" : "to-rose-600"}
          shadowColor={
            cashPickup >= 0 ? "shadow-orange-200" : "shadow-rose-200"
          }
        />
        <GradientCard
          title="Pick Up Amount"
          amount={pickup}
          loading={loading}
        ...
        <GradientCard
          ...
          loading={loading}
          isNegative={actualVariance < 0}
          gradientFrom={actualVariance >= 0 ? "from-teal-400" : "from-rose-500"}
          gradientTo={actualVariance >= 0 ? "to-emerald-500" : "to-red-600"}
          shadowColor={
            actualVariance >= 0 ? "shadow-teal-200" : "shadow-red-200"
          }
        />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="flex relative">
          <div
            className={`w-full rounded-2xl p-5 sm:p-6 text-white text-center flex flex-col justify-center items-center relative overflow-hidden ${
              finalCashInHand >= 0
                ? "bg-gradient-to-br from-amber-400 to-orange-500 shadow-orange-200"
                : "bg-gradient-to-br from-red-500 to-rose-700 shadow-rose-200"
            }`}
          >
            <div className="relative z-10 w-full">
              ...
              <div className="text-[10px] sm:text-[11px] font-bold bg-white/20 px-3 py-1.5 rounded-full backdrop-blur-sm shadow-sm border border-white/10 inline-flex items-center justify-center max-w-full">
                ...
                <span className="font-mono ml-1">${fmt(openingBalance)}</span>
              </div>
            </div>
