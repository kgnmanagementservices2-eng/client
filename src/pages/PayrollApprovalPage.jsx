...
export default function PayrollApprovalPage({ navParams }) {
  ...
  const canEdit = ["admin", "super_admin", "payroll_manager"].includes(
    user?.role,
  );

  // Ensure strict String comparison so the Market ID maps successfully
  ...
  const editMethods = useForm({
    values: editingRowData || {}, // 🔥 Reactive binding so it auto-fills perfectly
    mode: "onChange",
  });
  ...
  const editCalculations = useMemo(
    () => calculatePayrollTotals(editFormValues),
    [editFormValues],
  );

  const editStoreId = watch("store_id");

  // 🔥 Protected Employee fetching that forces value setting AFTER options load
  useEffect(() => {
    let active = true;
    if (editStoreId) {
      api
        ...
        .then((res) => {
          ...
          if (editingRowId) {
            ...
            if (
              ...
              // 🔥 Give React 50ms to physically render the <option> tags!
              setTimeout(() => {
                if (active) {
                  editMethods.setValue(
                    "employee_id",
                    Number(currentRow.employee_id), // Force as strict Number
  ...
  const fetchFiltered = useCallback(async () => {
    ...
    try {
      ...
      // 🔥 THE FIX: Use the master list from the backend!
      if (selectedDatePeriod === "all") {
        setAvailableDatePeriods(
          response.summary?.availableDatePeriods?.length > 0
            ? response.summary.availableDatePeriods
            : [
      ...
      dataRows.forEach(
        (r) =>
          (pt += num(
            r.net_final_pay !== undefined
              ? r.net_final_pay
              : (r.amount ?? r.amount_numeric ?? 0),
          )),
      );
      setPageTotal(pt);
    } catch (err) {
  ...
  const confirmAction = async () => {
    ...
    if (!trimmedReason && type === "reject")
      return toast.error(
        `You must provide a reason to reject this payroll record.`,
      );
    ...
    try {
      if (type === "approve")
        await api.approvePayrollExpense(
          id,
          trimmedReason,
          row.date,
          targetMarket,
        ...
        await api.rejectPayrollExpense(
          id,
          trimmedReason,
          row.date,
          targetMarket,
      ...
      await api.auditPayrollExpense(id, row.date, targetMarket);
  ...
  const openEditModal = (row) => {
    ...
    // 🔥 Safely parse all numbers so <input type="number"> doesn't fail
    ...
    // 🔥 Bind to reactive state instead of reset()
    setEditingRowData({
      ...sanitizedRow,
  ...
  const onEditSubmit = async (data) => {
    ...
    try {
      ...
      setEditingRowData(null); // 🔥 Safely clear memory
      fetchFiltered();
    } catch (err) {
      if (
        err?.message === "Book closed for the month. Contact admin." ||
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 animate-in zoom-in-95 duration-200">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className={`px-4 py-2 text-white rounded-lg text-sm font-bold shadow-sm transition-colors ${actionModal.type === "approve" ? "bg-emerald-600 hover:bg-emerald-700" : "bg-rose-600 hover:bg-rose-700"}`}
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl my-auto relative animate-in fade-in zoom-in-95 duration-200">
            <button
              onClick={() => {
                setIsEditModalOpen(false);
                setEditingRowData(null); // 🔥 Clear state on manual close
              }}
        ...
        <div className="grid gap-4 grid-cols-2 sm:grid-cols-4 lg:grid-cols-9 items-end">
          <div>
            ...
            <input
              ...
              className="border border-slate-200 rounded-md px-3 py-2 text-sm w-full bg-slate-50 text-slate-500 font-medium"
              disabled
            />
          </div>
          <div>
