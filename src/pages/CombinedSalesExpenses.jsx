...
export default function CombinedSalesExpenses({ onNavigate }) {
  ...
  const handleExport = () => {
    ...
    const headers = ["Date", "Market", "Store"];
    if (!isMarketManager) {
      headers.push("Cash (Sales)", "POS Cash", "POS Card", "QPay");
    }

    headers.push("Store Expenses", "Payroll", "Commission", "Total Expenses");

    if (!isMarketManager) {
      headers.push("Net", "Cash in Bank");
    }
    ...
    rows.forEach((r) => {
      const rowData = [r.date, `"${r.market || ""}"`, `"${r.store || ""}"`];

      if (!isMarketManager) {
        rowData.push(r.sales_total, r.pos_cash, r.pos_card, r.qpay);
      }
      ...
      if (!isMarketManager) {
        rowData.push(r.net, r.cash_in_bank);
      }

      rowData.push(r.variance);
          ...
          <div className="lg:col-span-2">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setUiYear(newYear);
                if (isMarketManager && newYear <= 2026 && uiMonth < 7) {
                  setUiMonth(7);
                }
              }}
            >
              {yearOptions.map((y) => (
                <option
                  key={y}
                  value={y}
                  disabled={isMarketManager && y < 2026}
                >
                  {y}
                </option>
              ))}
            ...
            >
              {monthOptions.map((m) => (
                <option
                  key={m.v}
                  value={m.v}
                  disabled={isMarketManager && uiYear <= 2026 && m.v < 7}
                >
                  {m.label}
                </option>
              ))}
