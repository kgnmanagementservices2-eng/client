...
export default function DashboardPage({ onNavigate }) {
  ...
  useEffect(() => {
    ...
    // Only attempt to render charts if the user is NOT a market manager
    ...
    if (!isSplitEmpty && chartRefs.split.current) {
      createChart("split", chartRefs.split.current.getContext("2d"), {
        ...
        options: {
          ...globalOptions,
          scales: {
            x: { ...globalOptions.scales.x, stacked: true },
            y: { ...globalOptions.scales.y, stacked: true },
          },
        },
      });
    }
    ...
    <section className="bg-slate-50 min-h-screen pl-4 pr-4 sm:p-6 lg:pl-6 pt-16 sm:pt-6 pb-4 sm:pb-6 space-y-8">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setYear(newYear);
                if (isMarketManager && newYear <= 2026 && month < 7) {
                  setMonth(7);
                }
              }}
            >
            ...
            >
              {monthOptions.map((m) => (
                <option
                  key={m.v}
                  value={m.v}
                  disabled={isMarketManager && year <= 2026 && m.v < 7}
                >
                  {m.label}
                </option>
              ))}
      ...
      {!loading && (
        <div className="space-y-8 animate-fadeIn">
          ...
          {/* 5. Charts Section - Hidden for Market Managers */}
          {!isMarketManager && (
